...
import com.softtech.kismiss.access.Calculation;
import com.softtech.kismiss.access.Group;
...
public class ReportTest {
	...
	public static void main(String args[]) throws Exception
	{
		...
		for(int i=0; i < 100; i++)
		{
			...
			employee.setSalary(123456D);
			employee.setCapacity(56);
...
//		List<SubTitle> subs = new ArrayList<SubTitle>();
		subs.add(new SubTitle("Print Date :"+new SimpleDateFormat("dd MMM yyyy hh:mm:ss").format(new Date()), "Print By : Hong Kisman", 250));
		subs.add(new SubTitle("Branch : 001", "DKI Jakarta", "Strategic Decision", 120 ));
//		subs.add(new SubTitle("Bisa Kan Kiri Kanan?", "Hehehe Jia You Deh", 120));
...
//		params.put(ReportFactory.BACKGROUND_URL, "D:/others/watermark.JPG");
		params.put(ReportFactory.TITLE, "SALE PRODUCTIVITY REPORT @ BY KISMAN HONG");
		params.put(ReportFactory.REPORT_NAME, "kismissTest.pdf");
		params.put(ReportFactory.SUB_TITLE, subs);
		
		report.setSubTitleFontSize(10);
//		report.setVisibleField(new String[]{"id","firstName","lastName","employee.firstName"});  //for dynamic field chosen
...
//		
		...
		Calculation calculation = new Calculation("phoneNumber", CalculationType.Sum, "#,##0", "Jumlah Login", VerticalAlignment.Middle, HorizontalAlignment.Right);
		calculations.add(calculation);
		Group group = new Group("country", calculations, 25, 25);
		report.viewReport(Person.class, persons, params, group);
